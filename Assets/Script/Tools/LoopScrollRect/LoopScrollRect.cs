...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        [Tooltip("Object cache pool")]
        public Transform CachePool;

        [Tooltip("Total count, negative means INFINITE mode")]
        public int totalCount;
        
        [Tooltip("Init object callback")]
        protected System.Action<Transform, int> InitCallback;
        ...
        public void ClearCells()
        {
            if (Application.isPlaying)
            {
                ...
                {
                    ReturnToPool(content.GetChild(i));
                }
            }
        }
        ...
        public void RefreshCells()
        {
            if (Application.isPlaying && this.isActiveAndEnabled)
            {
                ...
                for (int i = 0; i < content.childCount; i++)
                {
                    ...
                    {
                        CallInitCallback(content.GetChild(i), itemTypeEnd);
                        itemTypeEnd++;
                    }
                    else
                    {
        ...
        public void RefillCellsFromEnd(int offset = 0)
        {
            ...
            {
                ReturnToPool(m_Content.GetChild(i));
            }
        ...
        {            
            RectTransform nextItem = GetObjectFromPool().GetComponent<RectTransform>();
            nextItem.transform.SetParent(content, false);
            nextItem.gameObject.SetActive(true);
            CallInitCallback(nextItem, itemIdx);
            return nextItem;
        }
        //==========LoopScrollRect==========
        ...
        protected GameObject GetObjectFromPool()
        {
            if (0 == CachePool.childCount)
            {
                GameObject newObject = Instantiate(prefabSource) as GameObject;
                newObject.transform.SetParent(CachePool, false);
            }
            return CachePool.GetChild(0).gameObject;
        }

        protected void ReturnToPool(Transform obj)
        {
            obj.SetParent(CachePool);
        }

        protected void CallInitCallback(Transform obj, int index)
        {
            if (null != InitCallback)
            {
                InitCallback(obj, index);
            }
            else
            {
                Debug.LogError("LoopScrollRect.CallInitCallback InitCallback is null");
            }
        }
        public void RegisterInitCallback(System.Action<Transform, int> callback)
        {
